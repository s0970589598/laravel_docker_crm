...
class GraphDataService
{
     ...
     * @return mixed
     */
    public function salesSumGraphData() {
        $sum = $this->getCalculateProducts()+$this->getCalculateSales()+$this->getCalculateFinances()+$this->getCalculateDeals();
        $salesSumGraphData = app()->chartjs
            ->name('salesSumGraphData')
            ->type('pie')
            ->size(['width' => $this->width, 'height' => $this->height])
            ->labels(['Products', 'Sales', 'Finances', 'Deal'])
            ->datasets([
                [
                    'backgroundColor' => ['#FF6384', '#36A2EB', '#36A222', '#360111'],
                    'hoverBackgroundColor' => ['#FF6384', '#36A2EB', '#36A222', '#360111'],
                    'data' => [$this->getCalculateProducts(), $this->getCalculateSales(), $this->getCalculateFinances(), $this->getCalculateDeals()]
                ]
            ])
            ->options([]);

        return $salesSumGraphData;
    }
